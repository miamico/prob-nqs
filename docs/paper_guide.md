...
= e^{a^\top S}\prod_{j=1}^M \sum_{h_j=\pm1}\exp\left(h_j\left(b_j + (W^\top S)_j\right)\right)
= e^{a^\top S}\prod_{j=1}^M 2\cosh!\left(b_j + (W^\top S)_j\right).
$$
...
* $pflip_{sq,i} \leftarrow \frac{1}{N_c}\sum_{k=1}^{N_c} e^{-4 I_i v_i}$,

they compute a population variance proxy:
$$
\mathrm{Varpop}_i = \frac{pflip_{sq,i} - (pflip_i)^2}{N_c},
$$
and then:
$$
...
* implement Algorithm S1 exactly as written (outer $N_s$, inner $N_c$, compute $pflip_i$, $pflip_{sq,i}$, $\Delta_i$),
...
* $bias_i$ (fixed-point if you want fidelity),
* adjacency lists with `(j, W_ij)` for each node i,
* current state $\sigma_i \in \{-1,+1\}$.
...
* accumulate $e^{-2 I_i v_i}$ and $e^{-4 I_i v_i}$ to form $pflip_i$ and $pflip_{sq,i}$. 
...
5. `dbm_dual_sampling.py`
   ...
   * compute $pflip$, $pflip_{sq}$, $\Delta$, and corrected off-diagonal energy contribution 
...
* Re-derive the dual sampling identity $p(v^{(i)})/p(v)=\mathbb{E}_{p(h,d|v)}\left[e^{-\Delta E_i}\right]$ and show why it yields unbiased local energies in the $N_c\to\infty$ limit.  
